<?php
	...
	/*
	takeout_loan_work.php
	
	Dork's Bank
	
	Takeout loan processing
	*/
	if (isset($_POST['cid'])) {
		$cid = $_POST['cid'];
		try{
			/*
			Returns all Customers depending on ID
			...
			*/
			...
			/*
			Returns the max Loan ID from Loan Table
			*/
			$sql2 = "SELECT MAX(loanid) as max FROM loan where 1";
	...
	if (isset($_POST['send'])) {
		$missing = array();
		$max = trim(filter_input(INPUT_POST, 'loanid', FILTER_DEFAULT));
		$cid = trim(filter_input(INPUT_POST, 'cid', FILTER_DEFAULT));
		
		//remBal
		$remBal = trim(filter_input(INPUT_POST, 'remBal', FILTER_DEFAULT));
		if (empty($remBal)) {
			$missing[] = 'remBal';
		}
		
		//apr
		$apr = trim(filter_input(INPUT_POST, 'apr', FILTER_DEFAULT));
		if (empty($apr)) {
			$missing[] = 'apr';
		}
		
		//term
		$term = trim(filter_input(INPUT_POST, 'term', FILTER_DEFAULT));
		if (empty($term)) {
			$missing[] = 'term';
		}
		
		//curpay
		$curpay = trim(filter_input(INPUT_POST, 'curpay', FILTER_DEFAULT));
		if (empty($curpay)) {
			$missing[] = 'curpay';
		}
		while(!$missing) {
			try {
				...
				Input:	- ':loanid' : Loan ID
						...
						- ':curpay' : Curpay
						- ':cid': Customer ID
				Output: Redirect to takeout_loan.php
				*/
				$sql = "INSERT INTO loan(loanid, remBal, apr, term, curpay, cid) VALUES (:loanid, :remBal, :apr, :term, :curpay, :cid)";
				$stmt = $conn->prepare($sql);
				$stmt->bindValue(':loanid',$max);
				$stmt->bindValue(':remBal', $remBal);
				$stmt->bindValue(':apr', $apr);
				$stmt->bindValue(':term', $term);
				$stmt->bindValue(':curpay',$curpay);
				$stmt->bindValue(':cid',$cid);
				$stmt->execute();
				header('Location: takeout_loan.php');
				exit;
			} catch (PDOException $e) { 
				echo $e->getMessage(); 
				exit;
   ...
   <div class="clearfix borderbox" id="page"><!-- column -->
	...
	<table class="table" style="position:center;">
		...
		<tr>
			...
			<td class="cell-r"><?php echo $row['fn']; $first = $row['fn'];?></td>
			<td class="cell-r"><?php echo $row['ln']; $last = $row['ln'];?></td>
		</tr>
    <?php } //endforeach loop ?>
    </table>
	
	<!--LOAN CREATION-->
	<center><form method="post" action="takeout_loan_work.php">
		<input style="width: 375px; height: 40px;" name="loanid" type="hidden" value=<?php echo "$max";?>>
		<input style="width: 375px; height: 40px;" name="cid" type="hidden" value=<?php echo "$cid";?>>
		<!--TOTAL LOAN AMOUNT-->
		<p>
			<label style="font-size: 150%;color: #0000FF;"> Total Loan Amount: </label>
			<br><br>
			<input style="width: 375px; height: 40px;" name="remBal" type="text" value=<?php echo "$remBal";?>
				<?php if (isset($remBal)) {
					echo 'value="' . htmlentities($remBal) . '"';
				} ?>>
		</p>
		<br>
		<!--APR-->
		<p>
			<label style="font-size: 150%;color: #0000FF;">APR: </label>
			<br><br>
			<input style="width: 375px; height: 40px;" name="apr" type="text" value=<?php echo "$apr";?>
				<?php if (isset($apr)) {
					echo 'value="' . htmlentities($apr) . '"';
				} ?>>
		</p>
		<br>
		<!--TERM-->
		<p>
			<label style="font-size: 150%;color: #0000FF;">Term: </label>
			<br><br>
			<input style="width: 375px; height: 40px;" name="term" type="text" value=<?php echo "$term";?>
				<?php if (isset($term)) {
					echo 'value="' . htmlentities($term) . '"';
				} ?>>
		</p>
		<br>
		<!--CURPAY-->
		<p>
			...
			<input style="width: 375px; height: 40px;" name="curpay" type="text" value=<?php echo "$curpay";?>
				<?php if (isset($curpay)) {
					echo 'value="' . htmlentities($curpay) . '"';
				} ?>>
		</p>
		<br>
		<p>
